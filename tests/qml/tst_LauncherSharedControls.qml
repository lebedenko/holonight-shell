...
TestCase {
    ...
    function test_browse_panel_uses_shared_recent_category_and_empty_components() {
        ...
        const recentHeading = findChild(panel, "launcherRecentHeading")
        const category = findChild(panel, "launcherCategoryDelegate-0")
        const emptyState = findChild(panel, "recentApplicationsEmptyState")
        verify(recent)
        verify(recentHeading)
        compare(recentHeading.font.family, AppearanceService.uiFont)
        verify(category)
        verify(emptyState)
    ...
    function test_search_filters_use_explicit_values_and_existing_adapter() {
        ...
        const filtersHeading = findChild(panel, "launcherFiltersHeading")
        verify(control)
        verify(filtersHeading)
        compare(filtersHeading.font.family, AppearanceService.uiFont)
